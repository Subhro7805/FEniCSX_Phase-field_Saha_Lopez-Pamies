...
facet_tag = boundary.boundary_marker(domain, loading)
...
vtx = VTXWriter(comm, "./results/mortar3ptunnotched_3D.bp", [u, z], engine="BP4")
...
while t-stepsize < T:
    ...
    Smax = (4*Fy)*Span/4*(6/D/H**2)                              # Maximum nominal stress for 3 point loading beam calculated using the reaction force at the loading surface. 

    # print results to a text file.
    if comm.rank==0:
        with open('./results/mortar3ptunnotched_3D.txt', 'a') as rfile:
            rfile.write("timestep = %s, u2 = %s, zmin = %s, zmax = %s, Fy = %s, Ry = %s, Smax = %s\n" % (str(t), str(t*maxdisp), str(zmin), str(zmax), str(4*Fy), str(4*Ry), str(Smax)))
