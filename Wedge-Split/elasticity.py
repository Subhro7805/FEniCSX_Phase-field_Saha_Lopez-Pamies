...
def fracture_energy(u, z, y, dz, z_prev, dx, Gc, mesh, data, sts, shs, Wts, Whs, delta):
    ...
    alpha1 = delta*Gc/shs/8/data.eps-2*Whs/3/shs
    alpha2 = delta*Gc/8/data.eps*np.sqrt(3)*(3*shs-sts)/shs/sts+2*Whs/np.sqrt(3)/shs-2*np.sqrt(3)*Wts/sts
    ce = alpha2*SQJ2 + alpha1*I1 - z*(1-ufl.sqrt(I1**2)/I1)*psi11

    #Balance of configurational forces PDE
    pen=10**4*(3*Gc/8/data.eps)*ufl.conditional(ufl.lt(delta,1),1, delta)
    Wv=pen/2*((abs(z)-z)**2 + (abs(1-z) - (1-z))**2 )*dx
    Wv2=ufl.conditional(ufl.le(z, 0.05), 1, 0)*100*pen/2*((1/4)*(abs(z_prev-z)-(z_prev-z))**2)*dx
   
    # fracture functional
    R_z = y*2*z*(psi11)*dx + y*(ce)*dx + 3*delta*Gc/8*(-y/data.eps + 2*data.eps*ufl.inner(ufl.grad(z),ufl.grad(y)))*dx + ufl.derivative(Wv,z,y) +  ufl.derivative(Wv2,z,y)
