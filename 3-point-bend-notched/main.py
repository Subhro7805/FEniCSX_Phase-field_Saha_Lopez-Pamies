...
domain = createMesh.mesh3pt(comm, L, H, D, Span, x_load, ac, notch_width, h)                # Create mesh and save as XDMF file.
...
def loading(x):
    return np.isclose(x[1], H) & (x[0]>=0) & (x[0]<x_load/2)                             # Loading is applied on the top surface of the right part of the beam, which has a length of x_load/2. If you are modeling the full domain, change the load accordingly.

def rightbottom(x):
    return np.isclose(x[1], 0) & (x[0]>Span/2-x_load/2) & (x[0]<Span/2+x_load/2)         # fix y displacement

def rightsupport(x):
    return np.isclose(x[1], 0) & (x[2]<2*h) & (x[0]>Span/2-2*h) & (x[0]<Span/2+2*h)      # fix x and z displacements on a small area at the bottom center of the beam to avoid rigid body motion. 

def Xsym(x):
    return np.isclose(x[0], 0)                                                           # symmetric boundary condition on the x=0 surface. If you are modeling the full domain, you don't need this boundary condition.
...
def crackfront(x):
    return (x[1]>ac-3*eps) & (x[1]<ac+1e-4) & (x[0]<notch_width/2+1e-4)                  # Define the crack tip region. We will apply phase-field boundary condition in this region to ensure the crack initiates from the notch tip.

def outer(x):
    return (x[0]>0.9*Span/2)                                                             # Define the outer domain. We will apply phase-field boundary condition in this region to ensure the phase-field is 1 (undamaged) throughout this region and does not evolve.
...
maxdisp = 0.15                                         # Maximum u2 applied at the loading surface. You can change this value accordingly. The actual displacement applied at the loading surface will be maxdisp*t, where t is the current time. So the loading will increase linearly with time until it reaches maxdisp at t=1.
...
facet_tag = boundary.boundary_marker(domain, loading, crackfront)
...
vtx = VTXWriter(comm, "./results/mortar3ptnotched_3D.bp", [u, z], engine="BP4")
...
while t-stepsize < T:
    ...
    z_crackfront = evaluate.evaluate_function(comm, domain, z, (0.0,ac+2*eps,0.0), bb_tree)
    ...
    # calculate reaction force at the loading surface and rightbottom surface where y-displacement is fixed.
    ...
    if comm.rank==0:
        with open('./results/mortar3ptnotched_3D.txt', 'a') as rfile:
            rfile.write("timestep = %s, u2 = %s, z_crackfront = %s, Fy = %s\n" % (str(t), str(t*maxdisp), str(z_crackfront), str(4*Fy)))
