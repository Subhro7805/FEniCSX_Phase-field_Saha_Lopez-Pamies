...
def mesh3pt(comm, L, H, D, Span, x_load, ac, notch_width, h):
    if comm.rank == 0:
        # create mesh
        gmsh.initialize()
        ...
        outer_box_dim_tags = gm.addBox(0, 0, 0, L/2, H, D/2)

        notch = gm.addBox(0, 0, 0, notch_width/2, ac, D/2)

        box_dim_tags = gm.cut([(3, outer_box_dim_tags)], [(3, notch)])
    ...
    with XDMFFile(comm, "./results_mesh/Mortar3ptnotched_mesh3D.xdmf", "w") as xdmf:
