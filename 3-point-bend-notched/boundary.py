...
def boundary_marker(mesh, loading, crackfront):
...
def bc_phasefield(mesh, V_z, outer, crackfront):
    fdim = mesh.topology.dim-1
    ...
    entity_crackfront = locate_entities(mesh, fdim, crackfront)
    dofs_crackfront = locate_dofs_topological(V_z, fdim, entity_crackfront)
    bc_crackfront = dirichletbc(default_scalar_type(0), dofs_crackfront, V_z)

    bc_z = [bc_outer, bc_crackfront]
    
    return bc_z

def bc_dirichlet(mesh, V_u, boundary_conditions, rightbottom, loading, rightsupport, Xsym, Zsym):
    ...
    bc_u = [bc_rightbottom, bc_rightsupport_x, bc_rightsupport_z, bc_Xsym, bc_Zsym]
